...
export const EntityWso2ApiDefinitionTab = () => {
  ...
  const typeStr = String(entity.metadata.annotations?.['wso2.com/api-type'] || entity.spec?.type || '').toUpperCase();
  ...
  if (!hasTabs) {
    if (!definitionStr || definitionStr === 'WSO2 API Document content placeholder') {
      return (
        <EmptyState
          title="No Definition"
          missing="data"
    ...
    return (
      <InfoCard>
        <ApiDefinitionViewer value={definitionStr} language={typeStr === 'GRAPHQL' ? 'graphql' : undefined} />
      </InfoCard>
    );
  }
    ...
    <InfoCard>
      <Box mb={2}>
        <Tabs
          ...
          textColor="primary"
        >
          <Tab label="API Definition" value="source" className={classes.tabRoot} />
          {isSoap && <Tab label="WSDL" value="wsdl" className={classes.tabRoot} />}
          {isApiProduct && <Tab label="Resources" value="resources" className={classes.tabRoot} />}
        </Tabs>
      </Box>

      {activeTab === 'source' && (
        <Box p={2}>
          {!definitionStr || definitionStr === 'WSO2 API Document content placeholder' ? (
      ...
      {activeTab === 'wsdl' && (
        <Box p={2}>
          ...
          {!wsdlContentState.loading && wsdlContentState.value?.text && (() => {
            ...
            return (
              <Box mt={1}>
                <Typography variant="h6" gutterBottom>WSDL Definition</Typography>
                <ApiDefinitionViewer value={wsdlText} />
              </Box>
            );
          })()}

          {!wsdlContentState.loading && wsdlContentState.value?.isZip && wsdlContentState.value?.blob && (
            <Box mt={1} p={4} border={1} borderColor="divider" borderRadius={4} textAlign="center" bgcolor="background.default">
              <Typography variant="body2" color="textSecondary" style={{ marginBottom: '16px' }}>
                This API's WSDL definition is provided as a ZIP archive containing multiple files.
              </Typography>
              <Button
                variant="contained"
                color="primary"
                onClick={() => {
                  const url = window.URL.createObjectURL(wsdlContentState.value!.blob!);
          ...
          {!wsdlContentState.loading && !wsdlContentState.value?.text && !wsdlContentState.value?.isZip && (
            <Box p={4} border={1} borderColor="divider" borderRadius={4} textAlign="center" bgcolor="background.default">
              <Typography variant="body2" color="textSecondary">
